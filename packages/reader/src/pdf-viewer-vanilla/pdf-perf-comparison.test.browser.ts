...
async function measurePDFjsPerformance(): Promise<PerformanceMetrics> {
  ...
  await waitFor(() => typeof window.pdfjsLib !== "undefined", 10_000);
  ...
  const targetElementLast = document.querySelector(
    `[data-page-num="${TOTAL_PAGES}"]`,
  ) as HTMLElement;
...
describe("PDF Viewer Performance Comparison", () => {
  it(
    "compares PDF.js vs PDFium performance",
    { timeout: 3000000 },
    async () => {
      ...
      console.log(
        `  Winner:  ${pdfjsMetrics.initTime < pdfiumMetrics.initTime ? "PDF.js" : "PDFium"} (${Math.abs(pdfjsMetrics.initTime - pdfiumMetrics.initTime).toFixed(2)}ms faster)\n`,
      );

      console.log("First Page Render Time:");
      console.log(
        `  PDF.js:  ${pdfjsMetrics.firstPageRenderTime.toFixed(2)}ms`,
      );
      console.log(
        `  PDFium:  ${pdfiumMetrics.firstPageRenderTime.toFixed(2)}ms`,
      );
      console.log(
        `  Winner:  ${pdfjsMetrics.firstPageRenderTime < pdfiumMetrics.firstPageRenderTime ? "PDF.js" : "PDFium"} (${Math.abs(pdfjsMetrics.firstPageRenderTime - pdfiumMetrics.firstPageRenderTime).toFixed(2)}ms faster)\n`,
      );

      console.log("Scroll to Page 10 Time:");
      console.log(`  PDF.js:  ${pdfjsMetrics.scrollToPage10Time.toFixed(2)}ms`);
      console.log(
        `  PDFium:  ${pdfiumMetrics.scrollToPage10Time.toFixed(2)}ms`,
      );
      console.log(
        `  Winner:  ${pdfjsMetrics.scrollToPage10Time < pdfiumMetrics.scrollToPage10Time ? "PDF.js" : "PDFium"} (${Math.abs(pdfjsMetrics.scrollToPage10Time - pdfiumMetrics.scrollToPage10Time).toFixed(2)}ms faster)\n`,
      );

      console.log("Scroll to Last Page Time:");
      console.log(
        `  PDF.js:  ${pdfjsMetrics.scrollToLastPageTime.toFixed(2)}ms`,
      );
      console.log(
        `  PDFium:  ${pdfiumMetrics.scrollToLastPageTime.toFixed(2)}ms`,
      );
      console.log(
        `  Winner:  ${pdfjsMetrics.scrollToLastPageTime < pdfiumMetrics.scrollToLastPageTime ? "PDF.js" : "PDFium"} (${Math.abs(pdfjsMetrics.scrollToLastPageTime - pdfiumMetrics.scrollToLastPageTime).toFixed(2)}ms faster)\n`,
      );

      if (pdfjsMetrics.memoryUsed && pdfiumMetrics.memoryUsed) {
        console.log("Memory Usage:");
        console.log(
          `  PDF.js:  ${(pdfjsMetrics.memoryUsed / 1024 / 1024).toFixed(2)}MB`,
        );
        console.log(
          `  PDFium:  ${(pdfiumMetrics.memoryUsed / 1024 / 1024).toFixed(2)}MB`,
        );
        console.log(
          `  Winner:  ${pdfjsMetrics.memoryUsed < pdfiumMetrics.memoryUsed ? "PDF.js" : "PDFium"} (${Math.abs((pdfjsMetrics.memoryUsed - pdfiumMetrics.memoryUsed) / 1024 / 1024).toFixed(2)}MB less)\n`,
        );
      }
      ...
      if (pdfjsMetrics.initTime < pdfiumMetrics.initTime) pdfjsWins++;
      else pdfiumWins++;
      if (pdfjsMetrics.firstPageRenderTime < pdfiumMetrics.firstPageRenderTime)
        pdfjsWins++;
      else pdfiumWins++;
      if (pdfjsMetrics.scrollToPage10Time < pdfiumMetrics.scrollToPage10Time)
        pdfjsWins++;
      else pdfiumWins++;
      if (
        pdfjsMetrics.scrollToLastPageTime < pdfiumMetrics.scrollToLastPageTime
      )
        pdfjsWins++;
      else pdfiumWins++;
      ...
      console.log(
        `${pdfjsWins > pdfiumWins ? "PDF.js" : "PDFium"} (Won ${Math.max(pdfjsWins, pdfiumWins)} out of 4 metrics)`,
      );
