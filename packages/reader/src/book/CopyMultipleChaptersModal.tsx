...
export const CopyMultipleChaptersModal = observer(
  ({ isOpen, navItems, onClose, onCopy }: CopyMultipleChaptersModalProps) => {
    ...
    // Refs to avoid re-attaching keyboard listener on every state change
    const selectedRef = useRef(selected);
    const isCopyingRef = useRef(isCopying);
    const focusedIndexRef = useRef(focusedIndex);
    const navLengthRef = useRef(navItems.length);
    ...
    // Auto-close modal if navItems become empty
    useEffect(() => {
      if (isOpen && navItems.length === 0 && !isCopying) {
        onClose();
      }
    }, [isOpen, navItems.length, isCopying, onClose]);
    ...
    useEffect(() => {
      ...
      const handleKeyDown = (e: KeyboardEvent) => {
        ...
        const isCopying = isCopyingRef.current;
        const focusedIndex = focusedIndexRef.current;
        const navLength = navLengthRef.current;
        ...
        if (e.key === "ArrowDown") {
          e.preventDefault();
          setFocusedIndex((prev) => Math.min(prev + 1, navLength - 1));
          return;
        }
    ...
    return createPortal(
      ...
      >
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="copy-multiple-chapters-title"
        >
          {/* Header */}
          <div className="px-6 py-4 border-b border-gray-200">
            <h2
              id="copy-multiple-chapters-title"
