    ...
    return (
      <div className="fixed bottom-4 left-4 z-10 bg-white rounded-lg shadow px-2 py-2 flex items-center gap-2">
        ...
        <button
          ...
          title="Reset to 100% (fit to width)"
        >
          100%
        </button>
      </div>
 ...
 *
 * 4. ZOOM & LAYOUT:
 *    - Viewport-based zoom (% of screen width), 100% = fit to container
 *    - Zoom logic managed by store (zoomIn, zoomOut, resetZoom)
 ...
 *    - +/= : Zoom in, -/_ : Zoom out, 0 : Reset to 100% (fit to width)
...
export const PdfViewer = observer(({ store }: PdfViewerProps) => {
  ...
  // Use the outer scroll container's width (minus padding) for accurate measurement
  // This avoids the circular dependency where content maxWidth depends on page width
  const getContentWidth = useCallback((): number => {
    if (!containerRef.current) return 0;
    // Use the outer scroll container width and subtract the padding (16px left + 16px right = 32px)
  ...
  // Observe the outer scroll container to avoid circular dependency with content max-width
  useEffect(() => {
    if (!containerRef.current) return;

    const handleWidthChange = (containerWidth: number) => {
      // Calculate available content width: container width minus padding (16px left + 16px right)
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        // Use contentRect.width which is the container's content area
  ...
  return (
    ...
    >
      ...
      {/* Loading overlay during initial page restoration only */}
      {/* Note: Don't show overlay for pendingScrollRestore (zoom operations) as pages are already loaded */}
