...
export class PdfReaderStore {
  ...
  // Hysteresis-based memory management: clean when hitting MAX, down to MIN
  memoryBudgetMaxBytes = 500 * 1024 * 1024; // 500MB - trigger cleanup
  memoryBudgetMinBytes = 256 * 1024 * 1024; // 256MB - target after cleanup
  private debug = false;
  ...
  /**
   * Get memory budget for display (uses max)
   */
  get memoryBudgetBytes(): number {
    return this.memoryBudgetMaxBytes;
  }
  ...
  private storeBitmap(pageNum: number, bitmap: ImageBitmap) {
    ...
    // Enforce memory budget if over limit
    // This is critical to prevent runaway memory usage when renders complete faster
    // than page changes occur (e.g., during fast scrolling with worker rendering)
  ...
  private storeTileBitmap(
    ...
    // Update tilesLoaded observable to trigger React re-render
    const pageData = this.getPageData(pageNum);
    if (pageData?.tilesLoaded) {
      pageData.tilesLoaded.add(tileIndex);
    }
   ...
   *
   * Note: We don't set hasFull to false here to avoid visual flicker.
   * The old bitmap stays visible (even at wrong resolution) until
   * the new one is ready. This provides a smoother zoom experience.
   *
  ...
  private invalidateCurrentWindow() {
    ...
    for (let page = start; page <= end; page++) {
      ...
      // Don't clear hasFull - keep old bitmap visible until new one is ready
      this.docState.setPageStatus(page, "idle");
    }
  }

  /**
   * Schedule memory budget enforcement with hysteresis
   *
   * Strategy (Hysteresis Pattern):
   * 1. If over MAX budget (500MB), enforce immediately - clean down to MIN (256MB)
   * 2. If under MAX, do nothing - let memory accumulate for smooth scrolling
   * 3. This avoids frequent cleanup overhead and FPS drops
   * 4. Gives ~244MB of headroom for smooth scrolling before next cleanup
   */
  private scheduleMemoryBudgetEnforcement() {
    const totalBytes = this.memoryBytes + this.bitmapBytes;

    // Only enforce when hitting MAX threshold (hysteresis upper bound)
    // This prevents frequent cleanup during normal scrolling
    if (totalBytes > this.memoryBudgetMaxBytes) {
    ...
    // Under MAX: no cleanup needed, let memory accumulate
    // This is the key to hysteresis - we don't clean until hitting MAX
  }

  /**
   * Enforce memory budget by evicting bitmaps (LRU policy with hysteresis)
   *
   * Hysteresis Strategy:
   * - Triggered when memory > MAX (500MB)
   * - Cleans down to MIN (256MB) target
   * - This gives ~244MB of headroom for smooth scrolling
   *
   * Eviction priority:
   * 1. Full bitmaps and tile bitmaps outside window (LRU)
   * 2. Full bitmaps and tile bitmaps inside window if still over MIN (emergency)
   */
  private enforceMemoryBudget() {
    const totalBytes = this.memoryBytes + this.bitmapBytes;
    if (totalBytes <= this.memoryBudgetMaxBytes) return;

    console.log(
      `[MEMORY] Over MAX budget: ${(totalBytes / 1024 / 1024).toFixed(1)} MB / ${(this.memoryBudgetMaxBytes / 1024 / 1024).toFixed(0)} MB, cleaning to ${(this.memoryBudgetMinBytes / 1024 / 1024).toFixed(0)} MB`,
    );
    ...
    console.log(
      `[MEMORY] Window: [${windowStart}-${windowEnd}], pages in docState: ${pages.length}, current: ${this.currentPage}`,
    );

    let evictedCount = 0;
    let evictedBytes = 0;

    // Helper to evict all bitmaps for a page (full or tiles)
    const evictPageBitmaps = (pageData: PageData): number => {
      let freed = 0;

      // Evict full bitmap if exists
      const key = this.getTileKey(pageData.pageNumber);
      const bitmap = this.bitmaps.get(key);
      if (bitmap) {
        const size = this.getBitmapSize(bitmap);
        this.bitmapBytes -= size;
        bitmap.close();
        this.bitmaps.delete(key);
        this.docState.setPageHasFull(pageData.pageNumber, false);
        freed += size;
      }

      // Evict tile bitmaps if this is a tiled page
      if (pageData.isTiled && pageData.tiles) {
        for (const tile of pageData.tiles) {
          const tileKey = this.getTileKey(pageData.pageNumber, tile.tileIndex);
          const tileBitmap = this.bitmaps.get(tileKey);
          if (tileBitmap) {
            const size = this.getBitmapSize(tileBitmap);
            this.bitmapBytes -= size;
            tileBitmap.close();
            this.bitmaps.delete(tileKey);
            freed += size;
          }
        }
        // Clear tile loaded status
        if (pageData.tilesLoaded) {
          pageData.tilesLoaded.clear();
        }
      }
      ...
      if (canvas) {
        const size = this.getCanvasSize(canvas);
        this.detachCanvas(pageData.pageNumber, canvas);
        this.canvases.delete(key);
        freed += size;
      }
    ...
    // First pass: evict bitmaps outside window down to MIN budget
    for (const page of pages) {
      // Stop when we reach MIN threshold (hysteresis lower bound)
      if (this.memoryBytes + this.bitmapBytes <= this.memoryBudgetMinBytes)
        break;
      ...
      const freed = evictPageBitmaps(page);
      if (freed > 0) {
        evictedCount++;
        evictedBytes += freed;
      }
    ...
    // Second pass: if still over MIN, evict inside window (emergency)
    if (this.memoryBytes + this.bitmapBytes > this.memoryBudgetMinBytes) {
      console.log(
        "[MEMORY] Still over MIN after first pass, evicting inside window",
      );
      for (const page of pages) {
        if (this.memoryBytes + this.bitmapBytes <= this.memoryBudgetMinBytes)
          break;
    ...
    console.log(
      `[MEMORY] Evicted ${evictedCount} pages (${(evictedBytes / 1024 / 1024).toFixed(1)} MB), remaining: ${this.bitmaps.size} bitmaps, ${((this.memoryBytes + this.bitmapBytes) / 1024 / 1024).toFixed(1)} MB`,
    );
  }
  ...
  applyViewportZoom(containerWidth: number) {
    ...
    // Update maxPageWidth after zoom to ensure container sizes correctly
    // In viewport zoom mode, all pages have the same width (container * zoom)
    const targetWidth = Math.round(containerWidth * this.zoomPercent);
    if (targetWidth > 0 && this.pageCount > 0) {
      this.maxPageWidth = targetWidth;
    }

    // If container width changed significantly (>1px), mark bitmaps stale and re-render
    // This handles window resize, DevTools open/close, etc.
    if (
      Math.abs(containerWidth - previousWidth) > 1 &&
      previousWidth > 0 &&
      this.pageCount > 0
    ) {
      console.log(
        `[STORE] Container width changed: ${previousWidth.toFixed(0)} -> ${containerWidth.toFixed(0)}, triggering re-render`,
      );
      this.markZoomStale();
   ...
   * In viewport-based zoom mode, 100% means page width = container width
   * @param containerWidth Optional container width (falls back to lastContainerWidth)
   * @note Caller should set pendingScrollRestore before calling this method
   */
