...
export class ReaderStore {
  ...
  private async convertCurrentChapter() {
    ...
    if (this.useHtmlMode) {
      ...
      const { inlineChapterHTML } = await import("../lib/inlineHtmlAssets");
      const { html, urlsToRevoke } = await inlineChapterHTML(
        this.epub,
        chapter,
      );
      const { BookHtmlView } = await import("../book/BookHtmlView");
