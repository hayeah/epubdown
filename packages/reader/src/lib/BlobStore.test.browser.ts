...
describe("BlobStore", () => {
  ...
  describe("put", () => {
    ...
    it("should store a Blob with a key", async () => {
      const blob = new Blob(["Hello, Blob!"], { type: "text/plain" });

      await expect(blobStore.put("blob-key", blob)).resolves.toBeUndefined();
    });

    it("should store a File with a key", async () => {
      const file = new File(["Hello, File!"], "test.txt", {
        type: "text/plain",
      });

      await expect(blobStore.put("file-key", file)).resolves.toBeUndefined();

      // Verify it can be retrieved as bytes
      const bytes = await blobStore.getBytes("file-key");
      ...
      // Verify it can be retrieved as Blob
      const blob = await blobStore.getBlob("file-key");
  ...
  describe("getBytes", () => {
    ...
    it("should retrieve stored Blob as Uint8Array", async () => {
  ...
  describe("getBlob", () => {
    it("should retrieve stored Blob unchanged", async () => {
    ...
    it("should convert ArrayBuffer to Blob", async () => {
      ...
      if (retrieved) {
        const text = await retrieved.text();
        expect(text).toBe("ArrayBuffer data");
      }
    });
