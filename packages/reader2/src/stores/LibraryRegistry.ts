...
import type { BookMetadata, LibraryConfig, LibraryStore } from "../lib/LibraryStore";
...
export class LibraryRegistry {
  ...
  storeFor(libraryId: string): LibraryStore | null {
    ...
    if (lib.type === "filesystem") {
      ...
      const store = new FilesystemLibraryStore(this.db, libraryId, handle);
  ...
  async addFilesystemLibrary(
    ...
    const lib: LibraryConfig = { id, name, type: "filesystem", createdAt: Date.now() };

    await this.db.exec("INSERT INTO libraries (id, name, type, created_at) VALUES (?, ?, ?, ?)", [
      lib.id,
      lib.name,
      lib.type,
      lib.createdAt,
  ...
  async loadBookById(bookId: number): Promise<{ blob: Blob; metadata: BookMetadata } | null> {
    const result = await this.db.query("SELECT library_id FROM books WHERE id = ?", [bookId]);
