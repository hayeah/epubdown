...
const argv = yargs(hideBin(process.argv))
	.option("redownload", {
		type: "string",
		describe:
			"Regex pattern to match titles for redownloading even if they exist",
	})
	.option("verbose", {
		alias: "v",
		type: "boolean",
...
async function main() {
	try {
		...
		const redownloadRegex = argv.redownload
			? new RegExp(argv.redownload, "i")
			: null;
		...
		for (const epub of publicEpubs) {
			...
			if (exists) {
				console.log(
					`Redownloading: ${epub.title} (matched redownload pattern)`,
				);
			} else {
				console.log(
					`Downloading: ${epub.title} by ${epub.author} from ${epub.source}`,
				);
			}
			...
			try {
				...
				if (!resp.ok) {
					console.error(
						`Failed to download ${epub.title}: ${resp.status} ${resp.statusText}`,
					);
					failed++;
					continue;
				}
